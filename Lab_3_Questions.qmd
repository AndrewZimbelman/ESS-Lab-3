...
inner_join(Colorado_data, by = 'fips')
  COVID_Data = COVID_Population_Data 
  COVID_Data|>
  group_by(CTYNAME)|>
...
county_cen = read_csv('https://raw.githubusercontent.com/mikejohnson51/csu-ess-330/refs/heads/main/resources/county-centroids.csv')
  ...
  group_by(state)|>
  summarise(wmX_c= (sum(LON*cases)/sum(cases)), wmY_c = (sum(LAT*cases)/sum(cases)), wmX_d = (sum(LON*deaths) / sum(deaths)), wmY_d= (sum(LAT*deaths)/sum(deaths)),cases= sum(cases),deaths=sum(deaths), ratio = (sum(cases) / sum(deaths)),date)|>
  arrange(date)|>
  ungroup()
